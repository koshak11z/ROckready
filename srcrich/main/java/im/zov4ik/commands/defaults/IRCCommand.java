...
import im.zov4ik.features.impl.misc.IRC;
import im.zov4ik.zov4ik;
import im.zov4ik.utils.client.chat.ChatMessage;
import im.zov4ik.utils.client.text.TextHelper;
...
public class IRCCommand extends Command {
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        IRC ircModule = zov4ik.getInstance().getModuleRepository().modules().stream()
    ...
    private void handlePrefixSelection(String prefixNumber) {
        String[] prefixes = {"pikmi", "labuba", "zapen", "boost", "zov4ik", "panda", "smiley", "bibi", "benena", "blyabuba"};
    ...
    private void displayPrefixList() {
        ...
        if (MinecraftClient.getInstance().player != null) {
            ...
            MinecraftClient.getInstance().player.sendMessage(Text.literal("5. ").append(ChatMessage.ircprefixzov4ik("")), false);
    ...
    private Text getPrefixText(String prefixName, String message) {
        switch (prefixName) {
            ...
            case "Рич":
                return ChatMessage.ircprefixzov4ik(message);
            case "Панда":
    ...
    private void sendSetPrefix(String prefix) {
        if (zov4ik.getInstance().getIrcManager().getClient() != null && zov4ik.getInstance().getIrcManager().getClient().isOpen()) {
            zov4ik.getInstance().getIrcManager().getClient().sendSetPrefix(prefix);
        }
    }

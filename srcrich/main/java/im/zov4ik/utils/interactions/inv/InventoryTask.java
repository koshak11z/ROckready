...
import im.zov4ik.utils.interactions.item.ItemTask;
...
public class InventoryTask implements QuickImports {
    public static boolean useInventoryMoveLegitMode() {
        InventoryMove inventoryMove = Instance.get(InventoryMove.class);
    ...
    public static void moveItem(int from, int to, boolean updateInventory) {
        if (useInventoryMoveLegitMode() && !InventoryFlowManager.isExecutingTask()) {
    ...
    public static void swapToOffhand(int slotId) {
        ...
        int normalizedSlot = slotId;
        if (normalizedSlot >= 0 && normalizedSlot <= 8) {
            normalizedSlot += 36;
        }
        clickSlot(normalizedSlot, 40, SlotActionType.SWAP, false);
    }
    ...
    public static void swapAndUse(Slot slot, Turns angle) {
        ...
        if (useInventoryMoveLegitMode()) {
            int selectedSlot = mc.player != null ? mc.player.getInventory().selectedSlot : 0;
            InventoryFlowManager.addUseItemTask(slot.id, selectedSlot, () -> PlayerInteractionHelper.interactItem(Hand.MAIN_HAND));
            return;
        }

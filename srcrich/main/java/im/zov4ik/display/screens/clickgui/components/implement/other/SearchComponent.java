...
public class SearchComponent extends AbstractComponent {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        rectangle.render(ShapeProperties.create(matrix, x + width - 15.0F, y + 4, 0.5f, height - 8)
                .color(new Color(155, 155, 155, 55).getRGB()).build());

        Fonts.getSize(20, Fonts.Type.ICONS).drawString(context.getMatrices(), "U", x + width - 12.0F, y + 3.7f, typing ? -1 : 0xFF878894);
        String displayText = text.equalsIgnoreCase("") && !typing ? "Search" : text;
        ScissorAssist scissor = zov4ik.getInstance().getScissorManager();
        scissor.push(matrix.peek().getPositionMatrix(), x + 1, y, width - 19, height);
        ...
        font.drawString(context.getMatrices(), displayText, x + 4 - xOffset, y + (height / 2) - 1.25F, typing ? -1 : 0xFF878894);
        scissor.pop();
        ...
        if (focused && (selectionStart == -1 || selectionStart == selectionEnd)) {
            ...
            rectangle.render(ShapeProperties.create(matrix, x + 4 - xOffset + cursorX, y + (height / 2) - 3.4F, 0.5F, 6.8F).color(-1).build());
        }
        if (dragging) {
    ...
    public boolean charTyped(char chr, int modifiers) {
        if (typing && Fonts.getSize(11).getStringWidth(text) < width - 25.0F) {
            deleteSelectedText();

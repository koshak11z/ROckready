package im.zov4ik.display.screens.clickgui.components.implement.autobuy.util;
    ...
    public static boolean hasThornsEnchantment(ItemStack stack) {
        ...
        for (RegistryEntry<Enchantment> entry : enchants.getEnchantments()) {
            ...
            if (enchantId != null) {
                ...
                if (lowerEnchantId.contains("thorns") || lowerEnchantId.contains("???")) {
                    return true;
                }
            }
        }
        ...
        if (lore != null) {
            for (Text line : lore.lines()) {
                ...
                if (loreStr.contains("thorns") || loreStr.contains("???")) {
                    return true;
                }
            }
        }
    ...
    public static boolean compareItem(ItemStack a, ItemStack b) {
        ...
        if (hasLore) {
            for (Text expected : bLoreComp.lines()) {
                ...
                if (expectedStr.contains("??????") || expectedStr.contains("killer")) {
        ...
        if (hasLore) {
            ...
            for (Text expected : expectedLore) {
                ...
                boolean isOriginalMarker = expectedStr.contains("???????????? ???????") || expectedStr.contains("?");

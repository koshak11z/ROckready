...
public class AutoTotem extends Module {
    ...
    public AutoTotem() {
        ...
        setup(healthThreshold, elytraHealth, crystalDistance, fallCheck, saveTaliks, returnItem);
    }
    ...
    private void executeDefaultSwap() {
        ...
        InventoryTask.swapToOffhand(totemSlot);
        ...
        switch (phase) {
            ...
            case SWAP_TOTEM -> {
                if (elapsed > 3) {
                    ...
                    if (!MC.player.isOnGround() && Math.abs(MC.player.getVelocity().x) + Math.abs(MC.player.getVelocity().z) > 0.002D) {
                        actionStartTime = System.currentTimeMillis();
                        return;
                    }

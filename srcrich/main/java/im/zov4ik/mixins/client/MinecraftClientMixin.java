package im.zov4ik.mixins.client;
...
import im.zov4ik.features.impl.combat.NoInteract;
import im.zov4ik.utils.client.window.WindowStyle;
import im.zov4ik.utils.client.window.WindowTitleAnimation;
...
public abstract class MinecraftClientMixin implements QuickImports {
    ...
    private void stop(CallbackInfo ci) {
        ...
        if (zov4ik.getInstance().isInitialized()) {
            try {
                ...
                zov4ik.getInstance().getFileController().stopAutoSave();
            }
        }
    }
    ...
    public void setScreenHook(Screen screen, CallbackInfo ci) {
        ...
        zov4ik.getInstance().getDraggableRepository().draggable().forEach(drag -> drag.setScreen(event));

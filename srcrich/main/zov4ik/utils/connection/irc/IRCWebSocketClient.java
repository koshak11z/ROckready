...
import im.zov4ik.commands.defaults.IRCCommand;
...
public class IRCWebSocketClient extends WebSocketClient {
    ...
    public void onOpen(ServerHandshake handshakedata) {
        sendGetPrefix();
        zov4ik.getInstance().getIrcManager().isConnecting.set(false);
    }
    ...
    private void handleMessage(String jsonMessage) {
        try {
            ...
            if (type.equals("text")) {
                ...
                isMuted = true;
                if (zov4ik.getInstance().isShowIrcMessages() && json.has("reason") && json.has("duration_minutes")) {
    ...
    void displayMessage(String prefix, String message, String author, String content) {
        if (MinecraftClient.getInstance().player == null || !zov4ik.getInstance().isShowIrcMessages()) return;
        Text prefixText;
        if (author.equals("HZeed") && author.equals("Silv4ik") && author.equals("Raze")) {
            ...
            switch (prefix) {
                ...
                case "boost":
                    ...
                    break;
                case "zov4ik":
                    prefixText = ChatMessage.ircprefixzov4ik("");
                    break;
                case "panda":

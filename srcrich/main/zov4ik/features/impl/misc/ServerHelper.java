...
import im.zov4ik.display.hud.CoolDowns;
...
import im.zov4ik.utils.display.color.GradientAssist;
...
public class ServerHelper extends Module {
    ...
    public void onRotationUpdate(RotationUpdateEvent e) {
        ...
        // InventoryMove Normal: свап предметов больше НЕ тормозит игрока и не трогает клавиши движения.
    ...
    private void startItemUse(Slot slot, ItemInfo info, boolean addStarPrefix) {
        ...
        // InventoryMove Normal: не запоминаем/не сбрасываем клавиши движения и не тормозим игрока.
        // Свап начинается сразу с фазы SWAP_TO_ITEM.
    ...
    private void processItemAction() {
        ...
        switch (actionState) {
                    ...
                    if (wasFromInventory) {
                        ...
                        if (originalSourceSlot >= 36 && originalSourceSlot < 45) {
                            ...
                            if (targetSlot != originalSourceSlot) {
                                InventoryTask.switchTo(originalSourceSlot);
                            }
                        }
                    }
            ...
            case SPEEDING_UP -> {
                // InventoryMove Normal: не тормозим и не разгоняем — просто завершаем действие.
                actionState = ActionState.IDLE;
                originalSlot = -1;
                targetSlot = -1;
                originalSourceSlot = -1;

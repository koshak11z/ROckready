...
public class AutoSwap extends Module {
    ...
    public void onKey(KeyEvent e) {
        if (e.isKeyDown(bind.getKey()) && swapPhase == SwapPhase.READY) {
            if (!InventoryTask.useInventoryMoveLegitMode()) {
                ...
                Slot slotToSwap = findValidSlot(s -> true);
                if (slotToSwap != null) {
                    startLegitSwap(slotToSwap);
                }
            }
        }
    }
    ...
    private void processLegitSwap() {
        ...
        switch (swapPhase) {
            case SLOWING_DOWN -> {
                ...
                if (elapsed > (mc.player.isOnGround() ? 3L : 35L)) {
            ...
            case WAITING_STOP -> {
                ...
                long airborneGraceMs = mc.player.isOnGround() ? 5L : 80L;
                if ((velocityX < 0.001 && velocityZ < 0.001 && mc.player.isOnGround()) || elapsed > airborneGraceMs) {
            ...
            case SWAP -> {
                if (playerFullyStopped) {
                    if (targetSlot != null) {
                        InventoryTask.swapToOffhand(targetSlot.id);
                    }
            ...
            case SPEEDING_UP -> {
                ...
                float speedupProgress = Math.min(1.0f, speedupElapsed / 3.0f);
    ...
    private void executeDefaultSwap() {
        ...
        if (validSlot != null) {
            InventoryTask.swapToOffhand(validSlot.id);

...
public class BackgroundComponent extends AbstractComponent {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        String point = " • ";
        DiscordManager discord = zov4ik.getInstance().getDiscordManager();
        zov4ik.getInstance().getScissorManager().push(matrix.peek().getPositionMatrix(), 0, 0, window.getScaledWidth(), window.getScaledHeight());
        ...
        zov4ik.getInstance().getScissorManager().pop();
    }
        ...
        if (MenuScreen.INSTANCE.getCategory() == ModuleCategory.CONFIGS && button == 0) {
            ...
            for (Map<String, Object> map : displayedConfigs) {
                ...
                if (Calculate.isHovered(mouseX, mouseY, configX + 162, configY + 35, 14, 15)) {
                    if (isDefaultTab) {
                        try {
                            File dir = new File(zov4ik.getInstance().getClientInfoProvider().clientDir(), "Custom");
                            File configFile = new File(dir, config + ".json");
                            String json = new String(Files.readAllBytes(configFile.toPath()));
                            File tempFile = new File(zov4ik.getInstance().getClientInfoProvider().configsDir(), "temp.json");
                            Files.write(tempFile.toPath(), json.getBytes());
                            zov4ik.getInstance().getFileController().loadFile("temp.json");
                            tempFile.delete();
    ...
    private List<Map<String, Object>> getCloudConfigs() {
        try {
            if (!zov4ik.getInstance().getCloudConfigClient().isConnected()) {
            ...
            String response = zov4ik.getInstance().getCloudConfigClient().sendAndWaitForResponse(message);
    ...
    private void createDefaultConfig() {
        ...
        String name;
        do {
            name = "zov4ikConfig" + String.format("%03d", random.nextInt(1000));
    ...
    private String getCurrentConfigJson() {
        ...
        try {
            zov4ik.getInstance().getFileController().saveFile("temp.json");
    ...
    private void saveCloudConfig(String name) {
        ...
        data.put("avatar_hash", zov4ik.getInstance().getDiscordManager().getAvatarId().toString());
    ...
    private void saveCloudConfigWithJson(String name, String json) {
        ...
        try {
            ...
            String message = gson.toJson(request);
            zov4ik.getInstance().getCloudConfigClient().sendAndWaitForResponse(message);
        } catch (Exception e) {

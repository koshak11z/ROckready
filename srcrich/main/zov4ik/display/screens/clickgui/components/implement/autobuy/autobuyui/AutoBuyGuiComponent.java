package im.zov4ik.display.screens.clickgui.components.implement.autobuy.autobuyui;
...
import im.zov4ik.display.screens.clickgui.components.implement.autobuy.originalitems.ItemRegistry;
import im.zov4ik.display.screens.clickgui.components.implement.autobuy.window.AutoBuyItemSettingsWindow;
import im.zov4ik.display.screens.clickgui.components.implement.other.StatusRender;
...
public class AutoBuyGuiComponent extends AbstractComponent {
    ...
    private void renderAllItems(DrawContext context, MatrixStack matrix, int mouseX, int mouseY, float delta) {
        ...
        if (!krushItems.isEmpty()) {
            renderCategoryHeader(matrix, listX, itemY, listWidth, "?????");
            itemY += 20f;
        ...
        if (!donatorItems.isEmpty()) {
            renderCategoryHeader(matrix, listX, itemY, listWidth, "Донат вещи");
            itemY += 20f;
    ...
    private void renderItem(DrawContext context, MatrixStack matrix, AutoBuyableItem item, float itemX, float itemY, int mouseX, int mouseY, float delta, int index) {
        ...
        Fonts.getSize(12, Fonts.Type.REGULAR).drawString(matrix, "????. ????: $" + item.getSettings().getBuyBelow(), itemX + 30, itemY + 23, ColorAssist.getText(0.65F));
//        Fonts.getSize(12, Fonts.Type.REGULAR).drawString(matrix, "????. ????: $" + item.getSettings().getSellAbove(), itemX + 30, itemY + 26, ColorAssist.getText(0.65F));
        Fonts.getSize(12, Fonts.Type.REGULAR).drawString(matrix, "???. ???-??: " + item.getSettings().getMinQuantity(), itemX + 30, itemY + 30, ColorAssist.getText(0.65F));

...
public class ModuleDescriptions {
    public static String getDescription(Module module) {
        ...
        if (module instanceof AncientXrayV2) return "Ищет древние обломки по кэшу загруженных чанков.";
        if (module instanceof AncientXray) return "Подсвечивает древние обломки через стены.";
        ...
        if (module instanceof FireFly) return "Добавляет светлячков вокруг игрока.";
        if (module instanceof ShaderESP) return "Обводит игроков визуальным ESP-эффектом.";
        if (module instanceof SoulESP) return "Показывает душу игрока после смерти.";
        if (module instanceof Wings) return "Рисует полупрозрачные крылья за спиной.";

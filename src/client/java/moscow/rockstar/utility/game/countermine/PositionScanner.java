...
implements IMinecraft {
    ...
    private final EventListener<ClientPlayerTickEvent> onTick = event -> {
        ...
        this.points.clear();
        for (Entity entity : PositionScanner.mc.world.getEntities()) {
            boolean dead;
            String modelJson;
            DisplayEntity.ItemDisplayEntity itemDisplay;
            String modelId;
            Box boundingBox = entity.getBoundingBox();
            if (boundingBox.maxX != boundingBox.minX || PositionScanner.mc.player.distanceTo(entity) < 2.0f || !entity.getName().getString().contains("\u043f\u0440\u0435\u0434\u043c\u0435\u0442\u0430")) continue;
            boolean hologramNearby = CMUtility.isHologramNearby(entity, PositionScanner.mc.world, 2.5);
            if (!(entity instanceof DisplayEntity.ItemDisplayEntity) || (modelId = CMUtility.getModelIdFromNbt((itemDisplay = (DisplayEntity.ItemDisplayEntity)entity).getItemStack(), (RegistryWrapper.WrapperLookup)PositionScanner.mc.player.getRegistryManager())) == null || (modelJson = CMUtility.findHashedModel(modelId)) == null) continue;
            boolean sex = false;
            if (!modelJson.contains("{\"elements\":[{\"from\":[7.765625,8.0,7.765625]") && !modelJson.contains("{\"elements\":[{\"from\":[7.765625,8.0,7.8828125]") && !modelJson.contains("{\"elements\":[{\"from\":[7.9375,7.796875,7.875]") && !modelJson.contains("{\"elements\":[{\"from\":[7.921875,7.7421875,7.8828125]") && !modelJson.contains("{\"elements\":[{\"from\":[7.8828125,7.6484375,7.8828125]") && !modelJson.contains("{\"elements\":[{\"from\":[7.9003906,7.9003906,7.8691406]") && !modelJson.contains("{\"elements\":[{\"from\":[7.90625,7.7421875,7.8828125]") && !modelJson.contains("{\"elements\":[{\"from\":[7.9453125,7.6484375,7.8828125]") && !modelJson.contains("{\"elements\":[{\"from\":[7.8984375,7.8984375,7.8671875]") || (dead = false)) continue;
                ...
                if (nbt.contains("transformation")) {
                    ...
                    if (transformation.contains("left_rotation")) {
                        ...
                        if (Rockstar.getInstance().getModuleManager().getModule(CounterMine.class).isMinDamage()) {
                            this.points.add(new Point(entity, x > -0.04f, hologramNearby, entity.getPos().add(0.0, (double)-0.4f, 0.0)));
                            continue;
                        }
                        this.points.add(new Point(entity, x > -0.04f, hologramNearby, tiltedPos));
                        continue;
                    }
                    this.points.add(new Point(entity, true, hologramNearby, entity.getPos().add(0.0, (double)0.4f, 0.0)));
                    continue;
                }
                this.points.add(new Point(entity, true, hologramNearby, entity.getPos().add(0.0, (double)0.4f, 0.0)));
                continue;
    ...
    public Vec3d calculateTiltedPoint(Vec3d entityPos, float x, float y, float z, float w, float heightOffset) {
        ...
        Vec3d upVector = new Vec3d(0.0, (double)adjustedOffset, 0.0);
    ...
    private Vec3d rotateVectorByQuaternion(Vec3d vector, float qx, float qy, float qz, float qw) {
        ...
        return new Vec3d((double)rotatedX, (double)rotatedY, (double)rotatedZ);
    }

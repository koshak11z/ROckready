...
IWindow {
    ...
    private void resizeFramebuffer() {
        if (this.needsNewFramebuffer()) {
            int targetWidth = Math.max((int)Math.floor((float)mw.getScaledWidth() * this.downscale), 1);
            int targetHeight = Math.max((int)Math.floor((float)mw.getScaledHeight() * this.downscale), 1);

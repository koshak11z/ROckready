...
implements IMinecraft {
    private final EventListener<HudRenderEvent> onHudRender = event -> {
        ...
        if (MenuCloseListener.mc.currentScreen == null) {
        ...
        if (!(MenuCloseListener.mc.currentScreen instanceof MenuScreen) && Rockstar.getInstance().getModuleManager().getModule(MenuModule.class).isEnabled()) {
            Rockstar.getInstance().getModuleManager().getModule(MenuModule.class).setEnabled(false);
        }
        if (menuScreen.getMenuAnimation().getValue() > 0.1f && !(MenuCloseListener.mc.currentScreen instanceof MenuScreen) && menuScreen.isClosing()) {

...
public class WorldElement extends InlineElement {
    ...
    protected void drawLeadingIcon(UIContext context, float x, float y, float size, ColorRGBA color) {
        Glyphs.pin(context, x, y, size, color);
    }

    @Override
    public void update(UIContext context) {
        super.update(context);
        this.cords.update(String.format("X %s  Y %s  Z %s",
                Math.round(WorldElement.mc.player.getX()),
                Math.round(WorldElement.mc.player.getY()),
                Math.round(WorldElement.mc.player.getZ())));

...
implements IMinecraft {
    ...
    public void setYaw(float yaw, float direction) {
        ...
        double angle = MathHelper.wrapDegrees((double)Math.toDegrees(EntityUtility.direction(direction, forward, strafe)));
        ...
        for (float predictedForward = -1.0f; predictedForward <= 1.0f; predictedForward += 1.0f) {
            for (float predictedStrafe = -1.0f; predictedStrafe <= 1.0f; predictedStrafe += 1.0f) {
                ...
                double difference;
                if (predictedStrafe == 0.0f && predictedForward == 0.0f || !((difference = Math.abs(angle - (predictedAngle = MathHelper.wrapDegrees((double)Math.toDegrees(EntityUtility.direction(yaw, predictedForward, predictedStrafe)))))) < (double)closestDifference)) continue;
    ...
    public void setYaw(float yaw) {
        ...
        this.setYaw(yaw, InputEvent.mc.player.getYaw());
    }

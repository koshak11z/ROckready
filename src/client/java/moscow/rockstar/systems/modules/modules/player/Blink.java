...
extends BaseModule {
    ...
    private final EventListener<Render3DEvent> event3d = e -> {
        if (this.display.isEnabled() && this.lastPos != null && (Blink.mc.options.getPerspective() != Perspective.FIRST_PERSON || !this.svo.isEnabled())) {
            ...
            Vec3d cameraPos = Blink.mc.gameRenderer.getCamera().getPos();
            ms.push();
            RenderSystem.setShader((ShaderProgramKey)ShaderProgramKeys.POSITION_COLOR);
            RenderSystem.disableCull();
            RenderSystem.enableBlend();
            RenderSystem.defaultBlendFunc();
            Draw3DUtility.renderOutlinedBox(ms, quadsBuffer, Blink.mc.player.getBoundingBox().offset(this.lastPos.subtract(Blink.mc.player.getPos())).offset(-cameraPos.x, -cameraPos.y, -cameraPos.z), ColorRGBA.WHITE.withAlpha(180.0f));
    ...
    public void onEnable() {
        ...
        this.packets.clear();
        this.lastPos = Blink.mc.player.getPos();
        this.timer.reset();
    ...
    public void onDisable() {
        ...
        for (Packet<?> p : this.packets) {
            Blink.mc.player.networkHandler.sendPacket(p);
        }
        this.replaying = false;
        this.packets.clear();
        this.lastPos = null;

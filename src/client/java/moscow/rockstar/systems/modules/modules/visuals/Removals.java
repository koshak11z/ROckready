...
extends BaseModule {
    ...
    private final EventListener<ClientPlayerTickEvent> onUpdateEvent = event -> {
        if (this.fov.isSelected()) {
            Removals.mc.options.getFovEffectScale().setValue(0.0);
        }
    };

    @Override
    public void onEnable() {
        this.oldFovEffectScale = (Double)Removals.mc.options.getFovEffectScale().getValue();
        super.onEnable();
    }

    @Override
    public void onDisable() {
        Removals.mc.options.getFovEffectScale().setValue(this.oldFovEffectScale);
        super.onDisable();
    }
